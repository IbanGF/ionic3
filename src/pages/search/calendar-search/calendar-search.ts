...
export class CalendarSearchPage {
  ...
  saveDate() {
    const data = this.selectedMaxDate ? { from: this.selectedMinDate.date, to: this.selectedMaxDate.date } : null;
  ...
  ionViewDidLoad() {
    ...
    if (data) {
      if (data.from) { this.selectedMinDate = { date: data.from }; this.arrivalFormat = this.selectedMinDate.date.format('DD MMM YYYY'); }
      if (data.to) { this.selectedMaxDate = { date: data.to }; this.departureFormat = this.selectedMaxDate.date.format('DD MMM YYYY'); }
    }
